...
import me.dunescifye.lunaritems.utils.MaterialGroups;
...
import static me.dunescifye.lunaritems.utils.MaterialGroups.stripWood;
...
public class BlockBreakListener implements Listener {
    ...
    public void onPlayerBlockBreak(BlockBreakEvent e) {
        ...
        mainhand: {
            ...
            if (b.getBlockData() instanceof Ageable ageable && b.getType() != Material.SUGAR_CANE && b.getType() != Material.CACTUS && item.getType() == Material.NETHERITE_HOE) {
                if (ageable.getAge() == ageable.getMaximumAge()) {
                    ...
                    switch (Objects.requireNonNull(itemID)) {
                        case "nexushoe" -> handleNexusHoe(p, NexusHoePyroFarmingXPChance);
                        case "nexushoemega" -> handleNexusHoe(p, NexusHoeMegaPyroFarmingXPChance);
                        case "nexushoeo" -> handleNexusHoe(p, NexusHoeOPyroFarmingXPChance);
                ...
                String autoSmelt = container.get(autoSmeltScoreKey, PersistentDataType.STRING);
            ...
            // Radius pickaxes
            if (radiusMiningDisabledWorlds.contains(p.getWorld().getName())) break mainhand;
            ...
            if (container.has(LunarItems.keyRadius, PersistentDataType.DOUBLE)) {
                ...
                if (container.has(LunarItems.keyDepth, PersistentDataType.DOUBLE)) {
                    ...
                    if (itemID.contains("jollyaxe")) {
                        ...
                        if (customDrop != null && !customDrop.isEmpty()) {
                            drops = drops.stream()
                              .map(drop -> {
                                  ...
                                  else if (current.startsWith("BIRCH_"))
                                      mat = Material.getMaterial("BIRCH_" + customDrop);
                                  else if (current.startsWith("DARK_OAK_"))
                                      mat = Material.getMaterial("DARK_OAK_" + customDrop);
                                  else if (current.startsWith("JUNGLE_"))
                                      mat = Material.getMaterial("JUNGLE_" + customDrop);
                                  else if (current.startsWith("SPRUCE_"))
                                      mat = Material.getMaterial("SPRUCE_" + customDrop);
                                  else if (current.startsWith("ACACIA_"))
                                      mat = Material.getMaterial("ACACIA_" + customDrop);
                                  else if (current.startsWith("MANGROVE_"))
                                      mat = Material.getMaterial("MANGROVE_" + customDrop);
                                  else if (current.startsWith("CHERRY_"))
                                      mat = Material.getMaterial("CHERRY_" + customDrop);
                                  else if (current.startsWith("PALE_OAK_"))
                                      mat = Material.getMaterial("PALE_OAK_" + customDrop);
                                  if (mat != null) {
                        ...
                        if (b.getBlockData() instanceof Door door && door.getHalf() == Bisected.Half.TOP)
                            drops.add(new ItemStack(door.getMaterial()));
            ...
            // Replace all drops with this drop
            replaceDrop = container.get(replaceDropScoreKey, PersistentDataType.STRING);
            if (replaceDrop != null) {
                Material mat = Material.getMaterial(replaceDrop.toUpperCase());
                if (mat != null) {
                    drops = drops.stream().map(
            ...
            // Convert one group of materials to another
            String materialGroupFrom = container.get(keyMaterialGroupFrom, PersistentDataType.STRING);
            if (materialGroupFrom != null && materialTo != null && MaterialGroups.materialMap.containsKey(materialGroupFrom.toUpperCase())) {
                Set<Material> materialGroup = MaterialGroups.materialMap.get(materialGroupFrom).keySet();
                Material matTo = Material.getMaterial(materialTo.toUpperCase());
                if (matTo != null) {
                    drops = drops.stream().map(
                        drop -> materialGroup.contains(drop.getType()) ? drop.withType(matTo) : drop)
                      .collect(Collectors.toList());
                }
            }
        ...
        */
        offhand: {
            ...
            if (meta == null) break offhand;
            PersistentDataContainer pdc = meta.getPersistentDataContainer();
            String itemID = pdc.get(keyEIID, PersistentDataType.STRING);
            if (itemID == null) break offhand;

            if (drops.isEmpty()) drops = b.getDrops(item);

            // Block Storage
            // Stores a specified material inside the item. Uses a list of materials separated by commas
            String blockStorage = pdc.get(new NamespacedKey("score", "score-blockstorage"), PersistentDataType.STRING);
            if (blockStorage != null) {
                for (String mat : blockStorage.split(",")) {
                    int addAmount = 0;

                    Iterator<ItemStack> iterator = drops.iterator();
                    while (iterator.hasNext()) {
                        ItemStack drop = iterator.next();
                        if (drop.getType().toString().equals(mat.toUpperCase())) {
                            addAmount += drop.getAmount();
                            iterator.remove();
                        }
                    }

                    if (addAmount > 0) e.setDropItems(false);

                    NamespacedKey key = new NamespacedKey("score", "score-" + mat.toLowerCase());
                    Double currentAmt = pdc.getOrDefault(key, PersistentDataType.DOUBLE, 0.0);
                    pdc.set(key, PersistentDataType.DOUBLE, currentAmt + addAmount);
                    offhandItem.setItemMeta(meta);
                }
            }
        }

        // Only custom drop if vanilla doesn't drop
        if (!drops.isEmpty()) {
            List<Item> items = dropAllItemStacks(world, loc, drops);
